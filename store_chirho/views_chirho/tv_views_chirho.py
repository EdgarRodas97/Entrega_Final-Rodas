...
class TvListOfferViewChirho(LoginRequiredMixin, ListView):
    template_name = "tv_chirho/tv_list_offer_chirho.html"
    model = PostChirho
    
    def get_context_data(self, *args, **kwargs):
        context_chirho = super().get_context_data(**kwargs)
        offers_chirho = PostChirho.objects.filter(post_type_chirho = PostChirho.PostTypeChirho.TV_CHIRHO).all()
